...
class SRGAN:
    ...
    def build_model(self):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            self.learning_rate = tf.train.cosine_decay_restarts(learning_rate=self.config.lr, global_step=self.global_step,
                                                                first_decay_steps=30, t_mul=1.0, m_mul=0.8,
                                                                alpha=0)
            self.d_optim = tf.train.AdamOptimizer(learning_rate=self.learning_rate, beta1=self.config.beta1,
                                              beta2=self.config.beta2).minimize(self.d_loss, var_list=self.d_vars)
            print('d_optim_restart')
    ...
    def train(self):
        ...
        # data/train/*.*
        data = glob(os.path.join('D:/test11111111111111/SRGAN-master/SRGAN-master/Target_cifar10', '*.*'))
        print("numbers of training data=", len(data))
        ...
        for epoch in range(self.config.epoches):
            ...
            for idx in range(batch_idxs):
                ...
                if counter < 2e4:
                    _, content_loss= self.sess.run([self.srres_optim, self.content_loss],
                                                          feed_dict={self.input_target: batch_x})
                    end_time = time.time()
                    print(
                        'epoch{}[{}/{}]:total_time:{:.4f},content_loss:{:4f}'.format(epoch, idx, batch_idxs,
                                                                                                 end_time - start_time,
                                                                                                 content_loss))
                    ...
                    #                                    feed_dict={self.input_target: batch_x})
                    _, d_loss, summaries,lr= self.sess.run([self.d_optim, self.d_loss, self.summaries, self.learning_rate], feed_dict={self.input_target:batch_x, self.global_step:epoch})
                    _, g_loss, summaries= self.sess.run([self.g_optim, self.g_loss, self.summaries], feed_dict={self.input_target:batch_x})
                    end_time = time.time()
    ...
    def sample(self,epoch, idx):
        ...
        sample_images, input_source = self.sess.run([self.fake, self.input_source], feed_dict={self.input_target:batch_x})
        if epoch > 190:
            for i in range(len(sample_images)):
                image_path = 'D:/test11111111111111/SRGAN-master\SRGAN-master/sample' + '/' + 'epoch%03d' % epoch +'number%03d' % i
                plt.imshow(sample_images[i], interpolation='nearest')
                plt.axis("off")
                plt.savefig(image_path)

        # save_images(sample_images, './{}/{}_sample_{}_{}.png'.format(self.config.sample_dir, self.config.val_set,epoch, idx))
        # save_images(input_source, './{}/{}_input_{}_{}.png'.format(self.config.sample_dir, self.config.val_set,epoch, idx))
        print('---------------------------------------')
    ...
    def test(self):
        ...
        test = glob(os.path.join('D:/test11111111111111/SRGAN-master/SRGAN-master/Target_cifar10', '*.*'))
        ...
        for i in range(len(batch_x)):
            ...
            for i in range(len(sample_image)):
                image_path = 'D:/test11111111111111/SRGAN-master\SRGAN-master/test' + '/' + 'number%03d' % i
                plt.imshow(sample_image[i], interpolation='nearest')
                plt.axis("off")
                plt.savefig(image_path)
